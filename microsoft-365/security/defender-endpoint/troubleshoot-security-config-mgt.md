---
title: Fejlfinding af onboardingproblemer i forbindelse med sikkerhedsadministration for Microsoft Defender for Endpoint
description: Fejlfinding af problemer, der kan opstå under onboarding af enheder ved hjælp af Sikkerhedsadministration til Microsoft Defender for Endpoint.
...
ms.locfileid: "64467923"
---
# <a name="troubleshoot-onboarding-issues-related-to-security-management-for-microsoft-defender-for-endpoint"></a>Fejlfinding af onboardingproblemer i forbindelse med sikkerhedsadministration for Microsoft Defender for Endpoint
...
Sikkerhedsadministration for Microsoft Defender for Endpoint er en funktion til enheder, der ikke administreres af en Microsoft Endpoint Manager, hverken Microsoft Intune eller Microsoft Endpoint Configuration Manager for at modtage sikkerhedskonfigurationer til Microsoft Defender for Endpoint direkte fra Endpoint Manager.
Du kan finde flere oplysninger om sikkerhedsadministration Microsoft Defender for Endpoint i [Administrer Microsoft Defender for Endpoint på enheder med Microsoft Endpoint Manager](/mem/intune/protect/mde-security-integration).

Du kan finde sikkerhedsadministration Microsoft Defender for Endpoint instruktioner til onboarding i [Microsoft Defender for Endpoint administration af sikkerhedskonfiguration](security-config-management.md)

Denne indbyggede onboarding er designet til at være problemfri og kræver ikke brugerinput. Men hvis du støder på problemer under onboarding, kan du få vist og foretage fejlfinding af fejl i Microsoft Defender for Endpoint platform.

> [!NOTE]
> Hvis du har problemer med onboardingflowet til nye enheder, skal du [Microsoft Defender for Endpoint](/mem/intune/protect/mde-security-integration#prerequisites) forudsætningerne og sørge for, at onboardinginstruktionerne følges.

Du kan finde flere oplysninger om klientanalysen i [Fejlfinding af sensorens tilstand ved Microsoft Defender for Endpoint Klientanalyse](/microsoft-365/security/defender-endpoint/overview-client-analyzer).
...
Via Microsoft Defender for Endpoint kan sikkerhedsadministratorer nu foretage fejlfinding af sikkerhedsadministration til Microsoft Defender for Endpoint onboarding.
...
:::image type="content" source="./images/device-inventory-mde-error.png" alt-text="Lagerside for enhed" lightbox="./images/device-inventory-mde-error.png":::

Hvis du vil se en liste over alle enheder, der ikke har fuldført sikkerhedsadministrationen Microsoft Defender for Endpoint onboardingprocessen, skal du filtrere tabellen efter **MDE-Error**.
...
:::image type="content" source="./images/secconfig-mde-error.png" alt-text="De filterkriterier, der er anvendt på enhedens lagerside" lightbox="./images/secconfig-mde-error.png":::
...
Overvej at køre Klientanalyse på slutpunkter, der ikke fuldfører sikkerhedsadministrationen for Microsoft Defender for Endpoint onboardingflow. Du kan finde flere oplysninger om klientanalysen i [Fejlfinding af sensorens tilstand ved Microsoft Defender for Endpoint Klientanalyse](overview-client-analyzer.md).

MDE Client Analyzer-outputfilen (MDE Client Analyzer Results.htm) kan indeholde vigtige fejlfindingsoplysninger:

- Kontrollér, at enhedens operativsystem er i området for Sikkerhedsadministration Microsoft Defender for Endpoint onboardingflow i **sektionen Generelle enhedsoplysninger**
- Kontrollér, at enheden er registreret som en Azure Active Directory oplysninger i **Detaljer om administration af enhedskonfiguration**
  ...
  :::image type="content" source="images/client-analyzer-results.png" alt-text="Resultater fra klientanalyse" lightbox="images/client-analyzer-results.png":::
...
Som en del af onboardingflowet til sikkerhedsadministration er det f.eks. påkrævet, at Azure Active Directory-lejer-id'et i din Microsoft Defender for Endpoint-lejer passer til SCP-lejer-id'et,  der vises i rapporternes detaljer om enhedskonfigurationsadministration. Hvis det er relevant, anbefaler rapportoutputtet at udføre denne bekræftelse.
...
:::image type="content" source="images/enrollment-status.png" alt-text="Den side, der viser status for tilmelding" lightbox="images/enrollment-status.png":::
...
|---|---|---|
|`5-9`,`11-12`, `26-33`|Generel fejl|Enheden blev startet for at Microsoft Defender for Endpoint. Der opstod dog en fejl i flowet til sikkerhedskonfigurationsstyring. Dette kan skyldes, at enheden ikke er [mødeforud stand til at Microsoft Defender for Endpoint administrationskanalen](security-config-management.md). Kørsel af [Klientanalyse på](https://aka.ms/BetaMDEAnalyzer) enheden kan hjælpe med at identificere den egentlige årsag til problemet. Hvis det ikke hjælper, skal du kontakte support.|
|`13-14`,`20`,`24`,`25`|Forbindelsesproblem|Enheden blev startet for at Microsoft Defender for Endpoint. Der opstod dog en fejl i administrationsflowet til sikkerhedskonfigurationen, som kan skyldes et forbindelsesproblem. Kontrollér, [Azure Active Directory og Microsoft Endpoint Manager slutpunkter](security-config-management.md#connectivity-requirements) åbnes i din firewall.|
|`10`,`42`|Generel fejl i hybrid joinforbindelse|Enheden blev startet for at Microsoft Defender for Endpoint. Der opstod dog en fejl i administrationsflowet til sikkerhedskonfiguration, og operativsystemet kunne ikke udføre hybrid joinforbindelse. Brug [Fejlfinding Azure Active Directory-enheder, der er forbundet til](/azure/active-directory/devices/troubleshoot-hybrid-join-windows-current) fejlfinding af hybrid joinfejl i OS-niveau.|
|`15`|Lejerens uoverensstemmelse|Enheden blev startet for at Microsoft Defender for Endpoint. Der opstod dog en fejl i administrationsflowet til sikkerhedskonfiguration, fordi dit Microsoft Defender for Endpoint-lejer-id ikke stemmer overens med dit Azure Active Directory-lejer-id. Sørg for, at Azure Active Directory dit lejer-id fra din Defender for Endpoint-lejer svarer til lejer-id'et i dit domænes SCP-post. Hvis du vil have mere at [vide, skal du foretage fejlfinding af onboardingproblemer i forbindelse med sikkerhedsadministration Microsoft Defender for Endpoint](troubleshoot-security-config-mgt.md).|
|`16`,`17`|Hybridfejl – Forbindelsespunkt for tjeneste|Enheden blev startet for at Microsoft Defender for Endpoint. Men service Connection Point-posten (SCP) er ikke konfigureret korrekt, og enheden kunne ikke blive forbundet til Azure AD. Dette kan skyldes, at SCP konfigureres til at deltage i Enterprise DRS. Sørg for, at SCP-posten peger på AAD og SCP er konfigureret til at følge bedste praksis. Du kan finde flere oplysninger [under Konfigurere et tjenesteforbindelsespunkt](/azure/active-directory/devices/hybrid-azuread-join-manual#configure-a-service-connection-point).|
|`18`|Certifikatfejl|Enheden blev startet for at Microsoft Defender for Endpoint. Der opstod dog en fejl i administrationsflowet til sikkerhedskonfigurationen på grund af en fejl i enhedscertifikatet. Enhedscertifikatet tilhører en anden lejer. Kontrollér, at der følges bedste fremgangsmåder, når du opretter [certifikatprofiler, der er tillid til](/mem/intune/protect/certificates-trusted-root#create-trusted-certificate-profiles).|
|`36`|LDAP API-fejl|Enheden blev startet for at Microsoft Defender for Endpoint. Der opstod dog en fejl i flowet til sikkerhedskonfigurationsstyring. Bekræft netværktopologien, og sørg for, at LDAP-API'en er tilgængelig for at udføre hybrid joinanmodninger.|
|`37`|Problem med lokal synkronisering|Enheden blev startet for at Microsoft Defender for Endpoint. Der opstod dog en fejl i flowet til sikkerhedskonfigurationsstyring. Prøv igen senere. Hvis det ikke hjælper, skal du se Fejlfinding [af objektsynkronisering med Azure AD Forbind synkronisering](/azure/active-directory/hybrid/tshoot-connect-objectsync).|
|`38`,`41`|DNS-fejl|Enheden blev startet for at Microsoft Defender for Endpoint. Der opstod dog en fejl i flowet til sikkerhedskonfigurationen på grund af en DNS-fejl. Kontrollér internetforbindelsen og/eller DNS-indstillingerne på enheden. De ugyldige DNS-indstillinger kan være på arbejdsstationens side. Active Directory kræver, at du bruger domæne-DNS for at fungere korrekt (og ikke routerens adresse). Få mere at vide under [Fejlfinding af onboardingproblemer i forbindelse med sikkerhedsadministration for Microsoft Defender for Endpoint](troubleshoot-security-config-mgt.md).|
|`40`|Problem med synkronisering af ur|Enheden blev startet for at Microsoft Defender for Endpoint. Der opstod dog en fejl i flowet til sikkerhedskonfigurationsstyring. Kontrollér, at uret er indstillet korrekt og er synkroniseret på den enhed, hvor fejlen opstår.|
...
1. Åbn programmet Redigeringsprogram til synkroniseringsregler fra menuen Start. Find reglen med navnet Ind fra **AD – Computer join på listen over regler**. **Vær opmærksom på værdien i kolonnen "Prioriteret" for denne regel.**
   ...
   :::image type="content" source="images/57ea94e2913562abaf93749d306dd6cf.png" alt-text="Redigeringsprogrammet til synkroniseringsregler" lightbox="images/57ea94e2913562abaf93749d306dd6cf.png":::
...
2. Med reglen **Ind fra AD – Computerforbindelse fremhævet** skal du vælge **Rediger**. I dialogboksen **Bekræftelse af reserveret regel** skal du vælge **Ja**.
   ...
   :::image type="content" source="images/8854440d6180a5580efda24110551c68.png" alt-text="Bekræftelsessiden for den reserverede regel" lightbox="images/8854440d6180a5580efda24110551c68.png":::
