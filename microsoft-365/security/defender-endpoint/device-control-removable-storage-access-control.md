---
title: Flytbare Microsoft Defender til enhedsstyring på slutpunkter Storage Adgangskontrol, flytbare lagringsmedier
description: En gennemgang af Microsoft Defender til slutpunkt
...
ms.locfileid: "63592411"
---
# <a name="microsoft-defender-for-endpoint-device-control-removable-storage-access-control"></a>Flytbare Microsoft Defender til endpoint-enhedsstyring Storage Access Control

**Gælder for:**
- [Microsoft Defender for Endpoint Plan 2](https://go.microsoft.com/fwlink/p/?linkid=2154037)

> [!NOTE]
> Administration Gruppepolitik og Intune OMA-URI/Brugerdefineret politikstyring for dette produkt er nu generelt tilgængelig (4.18.2106): Se [Tech Community-blog: Beskyt dit flytbare lager og din printer med Microsoft Defender til slutpunkt](https://techcommunity.microsoft.com/t5/microsoft-defender-for-endpoint/protect-your-removable-storage-and-printers-with-microsoft/ba-p/2324806).


Microsoft Defender for Endpoint Device Control Flytbart Storage Access Control gør det muligt for dig at udføre følgende opgave:
...
|---|---|---|---|
|Oprettelse af flytbare mediegrupper|Giver dig mulighed for at oprette flytbare mediegrupper|Trin 1 og 3 i sektionen [Implementeringspolitik via OMA-URI](#deploying-policy-via-oma-uri) | Trin 1 i afsnittet [Installationspolitik via Gruppepolitik](#deploying-policy-via-group-policy)|
|Oprettelse af politik|Giver dig mulighed for at oprette en politik for at håndhæve hver flytbare mediegruppe|Trin 2 og 3 i sektionen [Implementeringspolitik via OMA-URI](#deploying-policy-via-oma-uri) | Trin 2 i afsnittet [Installationspolitik via Gruppepolitik](#deploying-policy-via-group-policy) |
|Standardhåndhævelse|Giver dig mulighed for at angive standardadgang (Afvisning eller Tillad) til flytbare medier, hvis der ikke er nogen politik|Trin 4 i afsnittet [Installationspolitik via OMA-URI](#deploying-policy-via-oma-uri) | Trin 3 i afsnittet [Installationspolitik via Gruppepolitik](#deploying-policy-via-group-policy) |
|Aktivér eller deaktiver Flytbare Storage Access Control|Hvis du indstiller Deaktiver, deaktiverer den politikken Flytbare Storage Access Control på denne computer| Trin 5 i afsnittet [Implementeringspolitik via OMA-URI](#deploying-policy-via-oma-uri) | Trin 4 i afsnittet [Installationspolitik via Gruppepolitik](#deploying-policy-via-group-policy) |
|Hente filoplysninger|Gør det muligt at oprette en politik for hentning af filoplysninger, når der sker skriveadgang| Trin 2 og 6 i sektionen [Implementeringspolitik via OMA-URI](#deploying-policy-via-oma-uri) | Trin 2 og 5 i sektionen [Installationspolitik via Gruppepolitik](#deploying-policy-via-group-policy) |
...
- **4.18.2111** eller nyere: Tilføj "Aktivér eller deaktiver Flytbare Storage Access Control", "Standardaktivering", opdateringstid for klientcomputerpolitik via PowerShell, filoplysninger

- **4.18.2201 eller** nyere: Understøtter en kopi af filen, der er skrevet til tilladt lagerplads via OMA-URI

:::image type="content" source="images/powershell.png" alt-text="Brugergrænsefladen i PowerShell.":::

> [!NOTE]
> Ingen af Windows Sikkerhed komponenter skal være aktive, da du kan køre Flytbare Storage Access Control uafhængigt Windows Sikkerhed status.
...
|---|---|---|
|**GroupId**|GUID, et entydigt id, repræsenterer gruppen og bruges i politikken.||
...
### <a name="access-control-policy"></a>Politik for adgangskontrol

| Egenskabsnavn | Beskrivelse | Indstillinger |
|---|---|---|
| **PolicyRuleId** | GUID, et entydigt id, repræsenterer politikken og bruges til rapportering og fejlfinding. | |
...
For at gøre dig bekendt med Microsoft Defender til slutpunkt Flytbar Storage Access Control har vi samlet nogle almindelige scenarier, du kan følge.
...
1. Kombiner alle grupper i `<Groups>` `</Groups>` én XML-fil.
    ...
    :::image type="content" source="images/prevent-write-access-allow-usb.png" alt-text="Skærmen viser de konfigurationsindstillinger, der tillader bestemte godkendte USBs på enheder.":::
...
3. Gem både regel- og gruppe XML-filer på netværkssharemappen, og sæt stien til netværkssharemappen  ind i Gruppepolitik-indstillingen **:** \> \> Administrative skabeloner til computerkonfiguration **Windows Komponenter** \> **Microsoft Defender Antivirus** \> **Enhedsstyring**: **"Definer** politikgrupper for enhedskontrol" og **'Definer politikregler for enhedsstyring'**.
   ...
   - Destinationscomputeren skal kunne få adgang til netværkssharen for at have politikken. Men når politikken er læst, er netværksshareforbindelsen ikke længere påkrævet, selv efter computeren er genstartet.
    ...
    :::image type="content" source="images/device-control.png" alt-text="Skærmbilledet Enhedshåndtering.":::
...
5. Aktivér eller deaktiver Flytbare Storage Adgangskontrol: Du kan indstille denne værdi til midlertidigt at deaktivere Flytbare Storage Adgangskontrol.
   ...
   - Når du installerer denne indstilling, får du vist **Aktiveret** eller **Deaktiveret**. Deaktiveret betyder, at denne computer ikke har flytbare Storage Access Control-politikken kørende.
...
Funktionen Flytbare Storage Access Control gør det muligt at anvende politikken via OMA-URI på enten en bruger eller enhed eller begge dele.
...
I forbindelse med politikinstallation i Intune skal kontoen have tilladelse til at oprette, redigere, opdatere eller slette enhedskonfigurationsprofiler. Du kan oprette brugerdefinerede roller eller bruge en af de indbyggede roller med disse tilladelser.
...
1. Opret en OMA-URI-regel for hver gruppe:
    ...
    - Datatype: Streng (XML-fil)
      ...
      :::image type="content" source="images/xml-data-type-string.png" alt-text="XML-filen for datatypen STRENG.":::
...
4. Aktivér eller deaktiver Flytbare Storage Adgangskontrol: Du kan indstille denne værdi til midlertidigt at deaktivere Flytbare Storage Adgangskontrol.
    ...
    **Deaktiveret** betyder, at denne computer ikke har flytbare Storage Access Control-politikken kørende
...
## <a name="deploying-and-managing-policy-by-using-intune-user-interface"></a>Implementering og administration af politik ved hjælp af brugergrænsefladen i Intune

Denne funktion findes i Microsoft Endpoint Manager Administration (<https://endpoint.microsoft.com/>). Gå til **Endpoint** **SecurityAttack** >  Surface **ReductionCreate** >  Policy. Vælg **Platform: Windows 10 og senere med** **Profil: Enhedshåndtering**.

## <a name="view-device-control-removable-storage-access-control-data-in-microsoft-defender-for-endpoint"></a>Vis flytbare enhedshåndteringsdata Storage Access Control-data i Microsoft Defender til slutpunkt

Portalen [Microsoft 365 Defender, der](https://security.microsoft.com/advanced-hunting) viser hændelser, der udløses af enhedskontrollens flytbare Storage Access Control. For at få Microsoft 365 adgang til sikkerhed skal du have følgende abonnement:
...
Den mest almindelige årsag er, at der ikke er nogen [påkrævet antimalwareklientversion](/microsoft-365/security/defender-endpoint/device-control-removable-storage-access-control#prepare-your-endpoints).

En anden årsag kan være, at XML-filen ikke er formateret korrekt, f.eks. hvis den ikke bruger den korrekte markdown-formatering for tegnet "&" i XML-filen, eller teksteditoren kan tilføje et byterækkefølgemærke (HTML) 0xEF 0xBB 0xBF i starten af filerne, hvilket får XML-parsing til ikke at fungere. En enkel løsning er at downloade [eksempelfilen](https://github.com/microsoft/mdatp-devicecontrol/tree/main/Removable%20Storage%20Access%20Control%20Samples) (vælg **Rå** og derefter **Gem som**) og derefter opdatere.

Hvis du installerer og administrerer politikken via Gruppepolitik, skal du sørge for at kombinere alle PolicyRule i én XML-fil i en overordnet node kaldet PolicyRules og alle grupper i én XML-fil i en overordnet node kaldet Grupper. Hvis du administrerer gennem Intune, skal du have én PolicyRule one XML-fil, den samme, én Gruppe med én XML-fil.
