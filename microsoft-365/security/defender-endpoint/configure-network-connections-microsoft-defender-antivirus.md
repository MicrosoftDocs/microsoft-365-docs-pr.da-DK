...
For at sikre at Microsoft Defender Antivirus skybaseret beskyttelse fungerer korrekt, skal dit sikkerhedsteam konfigurere dit netværk, så det tillader forbindelser mellem dine slutpunkter og visse Microsoft-servere. Denne artikel indeholder en liste over forbindelser, der skal være tilladt ved brug af firewallreglerne. Den indeholder også instruktioner til validering af forbindelsen. Hvis du konfigurerer din beskyttelse korrekt, sikrer du, at du får den bedste værdi fra dine skybaserede beskyttelsestjenester.

> [!IMPORTANT]
> Denne artikel indeholder oplysninger om, hvordan du kun konfigurerer netværksforbindelser til Microsoft Defender Antivirus. Hvis du bruger Microsoft Defender for Endpoint (hvilket omfatter Microsoft Defender Antivirus), skal du se [Konfigurer indstillinger for enhedsproxy og internetforbindelse for Defender for Endpoint](configure-proxy-internet.md).


> [!NOTE]
> Demowebstedet Defender for Endpoint på demo.wd.microsoft.com frarådes og fjernes fremover.

## <a name="allow-connections-to-the-microsoft-defender-antivirus-cloud-service"></a>Tillad forbindelser til den Microsoft Defender Antivirus cloudtjeneste

Den Microsoft Defender Antivirus cloudtjeneste giver hurtig og stærk beskyttelse af dine slutpunkter. Det er valgfrit at aktivere den skybaserede beskyttelsestjeneste. Microsoft Defender Antivirus cloudtjeneste anbefales, da den giver vigtig beskyttelse mod malware på dine slutpunkter og netværk. Du kan få flere oplysninger under [Aktivér skybaseret beskyttelse](enable-cloud-protection-microsoft-defender-antivirus.md) for at aktivere tjenesten med Intune, Microsoft Endpoint Configuration Manager, Gruppepolitik, PowerShell-cmdlet'er eller individuelle klienter i Windows Sikkerhed-appen.
...
|---|---|
|Microsoft Defender Antivirus cloudbaseret beskyttelsestjeneste kaldes Microsoft Active Protection Service (MAPS).<p> Den Microsoft Defender Antivirus bruger MAPS-tjenesten til at yde skybaseret beskyttelse.|`*.wdcp.microsoft.com` <p> `*.wdcpalt.microsoft.com` <p> `*.wd.microsoft.com`|
...
|Liste over tilbagekaldte certifikater (CRL) <p> Windows bruge denne liste, mens du opretter SSL-forbindelsen til MAPS for at opdatere listen over tilbagekaldte certifikater.|`http://www.microsoft.com/pkiops/crl/` <p> `http://www.microsoft.com/pkiops/certs` <p> `http://crl.microsoft.com/pki/crl/products` <p> `http://www.microsoft.com/pki/certs`|
|Symbol Store <p>Microsoft Defender Antivirus bruge symbol Store til at gendanne visse kritiske filer under afhjælpningsflowene.|`https://msdl.microsoft.com/download/symbols`|
|Universel GDPR-klient <p> Windows bruge denne klient til at sende klientdiagnosticeringsdata. <p> Microsoft Defender Antivirus bruger den generelle forordning om databeskyttelse til produktkvalitet og overvågning.|Opdateringen bruger SSL (TCP Port 443) til at downloade manifester og overføre diagnosticeringsdata til Microsoft, der bruger følgende DNS-slutpunkter: <p> `vortex-win.data.microsoft.com` <p> `settings-win.data.microsoft.com`|


## <a name="validate-connections-between-your-network-and-the-cloud"></a>Valider forbindelser mellem dit netværk og cloudmiljøet

Når du har tilladt de angivne URL-adresser, skal du teste, om du har forbindelse til Microsoft Defender Antivirus cloudtjenesten. Test, at URL-adresserne rapporterer korrekt og modtager oplysninger for at sikre, at du er fuldt beskyttet.
...
Brug følgende argument med Microsoft Defender Antivirus kommandolinjeværktøj (`mpcmdrun.exe`) til at kontrollere, at dit netværk kan kommunikere med Microsoft Defender Antivirus cloudtjenesten:
...
3. Under afsnittet **Karantænetrusler** skal du vælge **Se hele historikken** for at se den registrerede falske malware.
   ...
   > [!NOTE]
   > Versioner af Windows 10 før version 1703 har en anden brugergrænseflade. Se [Microsoft Defender Antivirus i appen Windows Sikkerhed](microsoft-defender-security-center-antivirus.md).
