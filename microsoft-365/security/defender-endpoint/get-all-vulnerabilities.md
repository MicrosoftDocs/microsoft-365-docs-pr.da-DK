---
title: Få alle sårbarheder
description: Henter en liste over alle sårbarheder, der påvirker organisationen
...
ms.locfileid: "63591828"
---
# <a name="list-vulnerabilities"></a>Liste over sårbarheder
...
Henter en liste over alle sårbarheder.
<br>[Understøtter OData V4-forespørgsler](https://www.odata.org/documentation/).
<br>OData-understøttede operatorer:
<br>```$filter```på: ```id```, ```name```, ```description```, ```cvssV3```, ```publishedOn```og ```severity``````updatedOn``` egenskaber.
