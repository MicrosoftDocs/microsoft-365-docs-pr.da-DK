...
5. Rul ned til sektionen **Overvåg eller begræns aktiviteter på Windows enhed**, og for hver aktivitet skal du angive den tilsvarende handling til **Bloker med tilsidesættelse**.
...
- Microsoft Word installeret på destinationscomputeren Windows 10
...
3. Opret et Word-dokument på den Windows 10 enhed, som politikken er målrettet til, anvend mærkaten, og kopiér det til de brugerkonti, der er lokale OneDrive mappe, der synkroniseres.  
...
3. Vælg **Tilføj eller rediger ikke-tilladte apps**, og tilføj *OneDrive* som et vist navn og det eksekverbare navn *onedrive.exe* for ikke at give onedrive.exe adgang til elementer med mærkaten **Meget fortroligt**.
...
7. Angiv stien til den mappe på lokale computere, hvor de oprindelige følsomme filer skal flyttes til. Eksempel:
    ...
    *C:\Users\IsaiahLanger\Microsoft DLP\Quarantine\OneDrive*
    ...
    > [!NOTE]
    > DLP Automatisk karantæne opretter undermapper til filerne for hver ikke-tilladte app. Så hvis du både har *Notesblok* og *OneDrive* på listen over ikke-tilladte apps, oprettes der en undermappe til **\OneDrive** og en anden undermappe til **\Notesblok**.
...
### <a name="configure-a-policy-to-block-onedrive-synchronization-of-files-with-the-sensitivity-label-highly-confidential"></a>Konfigurer en politik for at blokere OneDrive synkronisering af filer med følsomhedsmærkaten Yderst fortroligt
...
7. Opret en regel med disse værdier:
    ...
    1.  **Handlinger** >  **Overvåg eller begræns aktiviteter på Windows enhederAccess** >  **by unallowed appsBlock** > . I dette scenarie skal du rydde alle andre aktiviteter.
...
1. Log på den Windows 10 computer med den brugerkonto, du har angivet i [Konfigurer en politik for at blokere OneDrive synkronisering af filer med følsomhedsmærkaten Meget fortroligt](#configure-a-policy-to-block-onedrive-synchronization-of-files-with-the-sensitivity-label-highly-confidential) trin 5.
...
4. Kopiér den fil, du lige har oprettet, til mappen OneDrive synkronisering. Der vises en toast med en brugermeddelelse, der fortæller dig, at handlingen ikke er tilladt, og at filen bliver sat i karantæne. For brugernavnet *Isaiah Langer* og et dokument med titlen *automatisk karantænedokument 1.docx* får du f.eks. vist denne meddelelse:

    ![Pop op til brugermeddelelse om forebyggelse af datatab, der angiver, at handlingen OneDrive synkronisering ikke er tilladt for den angivne fil, og at filen vil blive sat i karantæne.](../media/auto-quarantine-user-notification-toast.png)
...
Når du vælger **Enheder** som en placering i en korrekt konfigureret DLP-politik og bruger Microsoft Edge browser, forhindres de ikke-tilladte browsere, du har defineret i disse indstillinger, i at få adgang til de følsomme elementer, der svarer til kontrolelementerne for din DLP-politik. Brugerne omdirigeres i stedet til at bruge Microsoft Edge som med den forståelse af DLP-begrænsninger, der er pålagt, kan blokere eller begrænse aktiviteter, når betingelserne i DLP-politikken er opfyldt.
...
3. Konfigurer DLP-politikker for at definere de typer følsomme elementer, som upload skal begrænses til disse steder, ved at aktivere **Upload til cloudtjenester** og **Adgang fra ikke-tilladte browsere**.
