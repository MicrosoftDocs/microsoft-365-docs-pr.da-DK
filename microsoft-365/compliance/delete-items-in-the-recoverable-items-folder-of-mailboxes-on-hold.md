...
Kør følgende kommando i [Security & Compliance PowerShell](/powershell/exchange/connect-to-scc-powershell) for at identificere den opbevaringspolitik, der anvendes på postkassen. Denne kommando returnerer også alle teams-politikker for opbevaring af samtaler, der er anvendt på en postkasse. Brug GUID 'et (inklusive ikke præfikset `mbx` eller `skp` ) for den opbevaringspolitik, du identificerede i trin 1.
