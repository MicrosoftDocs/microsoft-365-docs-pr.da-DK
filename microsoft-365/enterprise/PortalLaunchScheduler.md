...
ms.locfileid: "64824130"
---
# <a name="launch-your-portal-using-the-sharepoint-portal-launch-scheduler"></a>Start portalen ved hjælp af SharePoint Portalstartstyring

En portal er et SharePoint kommunikationswebsted på intranettet, der er i høj trafik – et websted, der har et sted fra 10.000 til over 100.000 seere i løbet af flere uger. Brug Portalstartstyring til at starte din portal for at sikre, at brugerne får en problemfri visningsoplevelse, når de får adgang til din nye SharePoint-portal.
<br>
<br>
...
- **Tovejs**: Start en ny moderne SharePoint-portal, der skal erstatte en eksisterende SharePoint klassisk eller moderne portal
...
> - Hvis din organisation bruger [Viva Connections](/SharePoint/viva-connections), kan brugerne muligvis se organisationens ikon på Microsoft Teams applinje, men når ikonet er valgt, kan brugerne ikke få adgang til portalen, før deres bølge er startet.
> - Denne funktion er ikke tilgængelig for Office 365 Tyskland, Office 365, der drives af 21Vianet (Kina) eller Microsoft 365 US Government-planer.
...
2. Start derefter planlægningen af portalens start ved at få adgang til Portal Launch Scheduler på en af to måder:
   ...
   **Mulighed 2**: Når som helst kan du navigere til startsiden for SharePoint kommunikationswebsted, vælge **Indstillinger** og derefter **planlægge start af webstedet** for at planlægge portalens start.

   ![Billede af ruden Indstillinger med Planlæg en start af et websted fremhævet.](../media/portal-launch-settings-2.png)

3. Derefter skal du bekræfte portalens tilstandsscore og foretage forbedringer af portalen, hvis det er nødvendigt, ved hjælp af [værktøjet Sidediagnosticering for SharePoint](https://aka.ms/perftool), indtil portalen modtager en score **for i orden**. Vælg derefter **Næste**.
...
8. Bekræft, at portalen starter detaljer, og vælg **Planlæg**. Når lanceringen er planlagt, skal eventuelle ændringer af SharePoint portalens startside modtage et sundt diagnosticeringsresultat, før portalen startes igen.
...
[Start SharePoint-portalen med 100.000 brugere](https://admin.microsoft.com/AdminPortal/?searchSolutions=Launch%20SharePoint%20Portal%20with%20100k%20users)
...
1. Hvis du vil redigere oplysninger om portalstart, **skal du** gå til Indstillinger og vælge **Planlæg webstedsstart**.
...
Værktøjet SharePoint Portal Launch Scheduler var oprindeligt kun tilgængeligt via [SharePoint PowerShell](/powershell/sharepoint/sharepoint-online/introduction-sharepoint-online-management-shell) og vil fortsat blive supporteret via PowerShell til kunder, der foretrækker denne metode. De samme noter i starten af denne artikel gælder for begge versioner af Portal Launch Scheduler.

> [!NOTE]
> Du skal have administratortilladelser for at bruge SharePoint PowerShell.
> Oplysninger om portalstart for de startr, der er oprettet i PowerShell, vises og kan administreres i det nye værktøj Portal launch scheduler i SharePoint.
...
1. [Download den nyeste SharePoint Online Management Shell](https://go.microsoft.com/fwlink/p/?LinkId=255251).
    ...
    >
    > Vælg dit sprog på siden Download Center, og klik derefter på knappen Download. Du bliver bedt om at vælge mellem at downloade en x64- og x86-.msi-fil. Download x64-filen, hvis du kører 64-bit versionen af Windows eller x86-filen, hvis du kører 32-bit versionen. Hvis du ikke ved det, kan du se [Hvilken version af Windows operativsystem kører jeg?](https://support.microsoft.com/help/13443/windows-which-operating-system). Når filen er downloadet, skal du køre den og følge trinnene i installationsguiden.

2. Forbind at SharePoint som [global administrator eller SharePoint administrator](/sharepoint/sharepoint-admin-role) i Microsoft 365. Du kan få mere at vide om, hvordan [du kommer i gang med SharePoint Online Management Shell](/powershell/sharepoint/sharepoint-online/connect-sharepoint-online).
...
Tovejsomdirigering omfatter lancering af en ny moderne SharePoint Online-portal, der skal erstatte en eksisterende SharePoint klassisk eller moderne portal. Brugere i aktive bølger omdirigeres til det nye websted, uanset om de navigerer til det gamle eller nye websted. Brugere i en ikke-lanceret bølge, der forsøger at få adgang til det nye websted vil blive omdirigeret tilbage til det gamle websted, indtil deres bølge er lanceret.
...
[Planlægning af udrulningsplanen for portalen i SharePoint Online](./planportallaunchroll-out.md)
